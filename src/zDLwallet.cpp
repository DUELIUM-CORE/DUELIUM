...
CzDLWallet::CzDLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zDL seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDLSeed_deprecated()) {
                    LogPrintf("%s: Updated zDL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zDL seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zDL wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDLCount(nCountLastUsed);
    else if (!walletdb.ReadZDLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDLWallet::Lock()
{
    seedMaster = 0;
}

void CzDLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDLWallet::LoadMintPoolFromDB()
{
...
void CzDLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zDLTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zDLTracker which also adds to database
    pwalletMain->zDLTracker->Add(dMint, true);
...
void CzDLWallet::SeedToZDL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDLWallet::GenerateDeterministicZDL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
