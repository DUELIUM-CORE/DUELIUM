...
public:
    ...
    bool WriteZDLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDLSeed_deprecated(uint256& seed);
    bool EraseZDLSeed();
    bool EraseZDLSeed_deprecated();

    bool WriteZDLCount(const uint32_t& nCount);
    bool ReadZDLCount(uint32_t& nCount);
