...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DL);
    ...
    if (!settings.contains("nAnonymizeDUELIUMAmount"))
        settings.setValue("nAnonymizeDUELIUMAmount", 1000);

    nAnonymizeDUELIUMAmount = settings.value("nAnonymizeDUELIUMAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDUELIUMAmount"))
        SoftSetArg("-anonymizeDUELIUMamount", settings.value("nAnonymizeDUELIUMAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in DUELIUM.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDUELIUMAmount:
            return QVariant(nAnonymizeDUELIUMAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDUELIUMAmount:
            nAnonymizeDUELIUMAmount = value.toInt();
            settings.setValue("nAnonymizeDUELIUMAmount", nAnonymizeDUELIUMAmount);
            emit anonymizeDUELIUMAmountChanged(nAnonymizeDUELIUMAmount);
            break;
        case CoinControlFeatures:
