...
std::set<std::string> ZDLControlDialog::setSelectedMints;
std::set<CMintMeta> ZDLControlDialog::setMints;

ZDLControlDialog::ZDLControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDLControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDLControlDialog::~ZDLControlDialog()
{
    delete ui;
}

void ZDLControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDLControlDialog::updateList()
{
...
void ZDLControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDLControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDL_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDLControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDLControlDialog::GetSelectedMints()
{
...
void ZDLControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
