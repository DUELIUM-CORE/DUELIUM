...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDL send
enum ZerocoinSpendStatus {
    ZDL_SPEND_OKAY = 0,                            // No error
    ZDL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDL_SPENT_USED_ZDL = 14,                      // Coin has already been spend
    ZDL_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZDL_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZDLBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDLOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzDLAuto) const;
    ...
    */
    ...
    CzDLWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDLTracker> zDLTracker;
    ...
    void setZWallet(CzDLWallet* zwallet)
    {
        zwalletMain = zwallet;
        zDLTracker = std::unique_ptr<CzDLTracker>(new CzDLTracker(strWalletFile));
    }

    CzDLWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDLAutoBackups(bool fEnabled)
    {
