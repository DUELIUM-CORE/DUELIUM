...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("DUELIUM Core DUELIUM-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  DUELIUM-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded DUELIUM transaction") + "\n" +
                               "  DUELIUM-tx [options] -create [commands]   " + _("Create hex-encoded DUELIUM transaction") + "\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded DUELIUM transaction
