...
#ifndef PHORE_ZDLWALLET_H
#define PHORE_ZDLWALLET_H
...
public:
    CzDLWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //PHORE_ZDLWALLET_H
