Contributing to DUELIUM Core
============================

The DUELIUM Core project operates an open contributor model where anyone is
...
The following applies to code changes to the DUELIUM Core project, and is not to be
confused with overall DUELIUM Network Protocol consensus changes.

Whether a pull request is merged into DUELIUM Core rests with the project merge
...
Patches that change DUELIUM consensus rules are considerably more involved than
...
Where a patch set proposes to change the DUELIUM consensus, it must have been
...
The project leader is the release manager for each DUELIUM Core release.
