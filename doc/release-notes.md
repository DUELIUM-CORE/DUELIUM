DUELIUM Core version 3.0.4 is now available from:

  <https://github.com/DUELIUM-CORE/DUELIUM/releases>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/DUELIUM-CORE/DUELIUM/issues>


Mandatory Update
==============

DUELIUM Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zDL protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/DUELIUM-Qt (on Mac) or DUELIUMd/DUELIUM-qt (on Linux).


Compatibility
==============

DUELIUM Core is extensively tested on multiple operating systems using
...
DUELIUM Core should also work on most other Unix-like systems but is not
...
- #294 `27c0943` Add additional checks for txid for zDL spend. (presstab)
- #301 `b8392cd` Refactor zDL tx counting code. Add a final check in ConnectBlock() (presstab)
- #306 `77dd55c` [Core] Don't send not-validated blocks (Mrs-X)
- #312 `5d79bea` [Main] Update last checkpoint data (Fuzzbawls)
- #325 `7d98ebe` Reindex zDL blocks and correct stats. (presstab)
- #327 `aa1235a` [Main] Don't limit zDL spends from getting into the mempool (Fuzzbawls)
...
- #316 `ed192cf` [Minting] Better filtering of zDL serials in miner. (presstab)

### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zDL (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when DUELIUMd --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] DUELIUM not responding on user quitting app (Aaron Langford)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/DUELIUM-project-translations/).
