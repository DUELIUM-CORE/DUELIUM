...
VERSION=master
commit=false
url=https://github.com/DUELIUM/DUELIUM
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the DUELIUM, gitian-builder, gitian.sigs, and DUELIUM-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/DUELIUM/DUELIUM
...
then
	# Make output folder
	mkdir -p "./DUELIUM-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit DUELIUM=${COMMIT} --url DUELIUM=${url} ../DUELIUM/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/DUELIUM-*.tar.gz build/out/src/DUELIUM-*.tar.gz ../DUELIUM-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit DUELIUM=${COMMIT} --url DUELIUM=${url} ../DUELIUM/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/DUELIUM-*.zip build/out/DUELIUM-*.exe ../DUELIUM-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit DUELIUM=${COMMIT} --url DUELIUM=${url} ../DUELIUM/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/DUELIUM-*.tar.gz build/out/DUELIUM-*.dmg ../DUELIUM-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit DUELIUM=${COMMIT} --url DUELIUM=${url} ../DUELIUM/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../DUELIUM/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/DUELIUM-*.tar.gz build/out/src/DUELIUM-*.tar.gz ../DUELIUM-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../DUELIUM/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/DUELIUM-*win64-setup.exe ../DUELIUM-binaries/${VERSION}
	    mv build/out/DUELIUM-*win32-setup.exe ../DUELIUM-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../DUELIUM/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/DUELIUM-osx-signed.dmg ../DUELIUM-binaries/${VERSION}/DUELIUM-${VERSION}-osx.dmg
	fi
	popd || exit
